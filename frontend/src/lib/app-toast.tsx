...
export const appToast = {
  pdfUploading: (subtitle?: string) =>
    toast.loading("Uploading Lab Manual", {
      id: "pdf-upload",
  ...
  pdfReady: (fileName: string, chunks?: number, freshChat?: boolean) =>
    toast.success("Lab Manual ready", {
      id: "pdf-upload",
      description:
        freshChat
          ? `${
            ...
            } Messages were removed from this view (Lab Manual stays loaded). Previous messages are still saved in Workspaces.`
          : chunks != null
            ? `${fileName} · ${chunks} chunks indexed`
            : `${fileName} is ready to chat about.`,
  ...
  allSessionsCleared: (count: number) =>
    toast.success("All workspaces cleared", {
      description: `Removed ${count} saved workspace${count === 1 ? "" : "s"} from this browser.`,
      icon: wrapIcon(<Trash2 className="text-slate-400" />),
    }),
  ...
  chatCleared: () =>
    toast.message("Chat cleared", {
      description: "Messages were removed from this view (Lab Manual stays loaded).",
      icon: wrapIcon(<Trash2 className="text-slate-400" />),
    }),
  ...
  sourcesEnabled: () =>
    toast.success("Sources on", {
      description:
        "When the model returns them, answers include short source citations from your lab manual.",
