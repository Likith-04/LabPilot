...
async def upload_pdf(
    ...
    """
    Upload and process a laboratory manual PDF.
    ...
    if not fname.lower().endswith(".pdf"):
        raise HTTPException(
            status_code=400,
            detail="Only laboratory manuals in PDF format are allowed",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error processing Lab Manual: {str(e)}",
        ) from e
...
async def get_status(
    ...
    Checks if a lab manual is loaded and ready for queries.
    """
    settings = get_settings()
    ...
    return StatusResponse(
        ...
        message="Lab Manual loaded and ready for questions"
        if is_ready
        else "No Lab Manual loaded. Please upload your laboratory manual first.",
        pdf_loaded=is_ready,
