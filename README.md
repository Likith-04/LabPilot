...
- Groq can be used for chat generation, but this code uses other providers or local `sentence-transformers` for embeddings.
- Set `EMBEDDING_OPENAI_DIRECT=true` only if you want direct OpenAI embeddings through `OPENAI_DIRECT_API_KEY`.
- For production, set `CORS_ORIGINS` to your deployed frontend URL.
